...
namespace Expenses.Umbraco.ValidationAttributes.Services
{
    public static class ValidationAttributesService
    {
        ...
        internal static string DictionaryValue(string dictionaryKey)
        {
            if (_umbracoHelperAccessor is null)
                throw new NullReferenceException("Expenses.Umbraco.ValidationAttributes: cannot access IUmbracoHelperAccessor. Please make sure everything is configured properly.");

            if (!_umbracoHelperAccessor.TryGetUmbracoHelper(out _umbracoHelper))
                throw new NullReferenceException("Expenses.Umbraco.ValidationAttributes: cannot access UmbracoHelper instance.");
